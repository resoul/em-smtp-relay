...
class SettingsPage
{
    ...
    public function addActionLinks(array $links): array
    {
        $settingsLink = sprintf(
            ...
            $this->localization->esc('Settings')
        );
    ...
    public function render(): void
    {
        if (!current_user_can('manage_options')) {
            wp_die(
                $this->localization->esc('You do not have sufficient permissions to access this page.'),
                $this->localization->esc('Permission Denied'),
                ['response' => 403]
            );
        }

        echo '<div class="wrap">';
        echo '<h1>' . $this->localization->esc('Emercury SMTP Settings') . '</h1>';
        echo '<p>' . wp_kses_post(sprintf(
                $this->localization->t('%s<a href="%s" target="_blank" rel="noopener">Emercury SMTP account</a>%s'),
                'Log into your ',
